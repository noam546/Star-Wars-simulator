...
public abstract class MicroService implements Runnable {
    ...
    public MicroService(String name) {
        this.name = name;
        myMessageBus = MessageBusImpl.getInstance();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        myMessageBus.subscribeEvent(type,this);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        myMessageBus.subscribeBroadcast(type,this);
        callBackMap.put(type,callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return myMessageBus.sendEvent(e);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        myMessageBus.complete(e,result);
    }
    ...
    public final void run() {
        ...
        try {
            while(!Thread.currentThread().isInterrupted()){
                Message commitEvent = myMessageBus.awaitMessage(this);
                if(commitEvent!=null) {
                    callBackMap.get(commitEvent.getClass()).call(commitEvent);
                }
            }
